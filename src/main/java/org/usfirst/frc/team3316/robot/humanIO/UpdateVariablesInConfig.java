  ...
  protected void initialize () {
    ...
    for (Map.Entry<String, Class<?>> entry : variables) {
      ...
      if (entry.getValue() == Double.class) {
        config.add(key, SmartDashboard.getNumber(key, 3316));
      } else if (entry.getValue() == Integer.class) {
        config.add(entry.getKey(), (int) SmartDashboard.getNumber(key, 3316));
      } else if (entry.getValue() == Boolean.class) {
        config.add(entry.getKey(), SmartDashboard.getBoolean(entry.getKey(), false));
      }
    }
  }
